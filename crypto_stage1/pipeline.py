"""Top-level orchestrator for Stage 0 -> Stage 1 -> Stage 2.
...
def run_pipeline(
    ...
    *,
    ...
    """Run the full pipeline and return Stage 2 outputs as dictionaries."""
    active = settings or Settings.from_env()
    embedding = EmbeddingService.from_settings(active)
    model = LLMClient.from_settings(active)
    ...
    stage2_outputs = run_stage2(stage2_inputs, client=model)
    return [asdict(output) for output in stage2_outputs]
